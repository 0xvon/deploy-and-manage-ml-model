...
def lambda_handler(event, context):
    ...
    # Get datetime strings for the dates in the interval we want to train on
    dates = get_dates(INTERVAL)
    # filenames = ['{}.csv'.format(date) for date in dates] # Convert datetime strings to filenames
    filenames = ['2019-10-10.csv']
    # Check to see if objects matching the filenames exist in S3
    extant_objects = check_objects_exist(filenames)
    # Find the date of the last data file uploaded
    latest_data_upload = get_latest_date(extant_objects)
    # Retrieve the date of the last successful training job from Parameter Store
    last_train_date = ssm.get_parameter(Name=LAST_TRAIN_PARAM)[
        'Parameter']['Value']
